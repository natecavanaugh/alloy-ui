AUI.add('aui-uploader', function(A) {
var Lang = A.Lang,
	...
	CSS_DROP_NODE = getClassName(NAME, 'drop', 'node'),
	CSS_DROP_NODE_OVER = getClassName(NAME, 'drop', 'node', 'over'),
	CSS_UPLOADER_HTML = getClassName(NAME, 'html'),
	CSS_UPLOADER_IMAGE = getClassName(NAME, 'image'),
	CSS_UPLOADER_LABEL = getClassName(NAME, 'label'),

	TPL_DROP_NODE = '<div class="' + [CSS_DROP_NODE, CSS_HELPER_HIDDEN].join(' ') + '"></div>',
	...
	TPL_SELECT_INPUT = '<label class="' + CSS_UPLOADER_LABEL + '"><input type="file" {0} /></label>',
...
var UploaderDD = A.Component.create(
	{
		...
		prototype: {
			...
			_appendInputFile: function(width) {
				...
				var input = A.Node.create(Lang.sub(TPL_SELECT_INPUT, [(instance.get('multiFiles') ? 'multiple="true"' : '')]));
			...
			_doAbortXHR: function(event) {
				...
				instance.fire(
					'uploadcancel',
					{
						fileName: uploaderDD.fileName || uploaderDD.file.name,
						id: fileId,
						type: 'uploadcancel'
					}
				);
			...
			_onDragLeave: function(event) {
				...
				instance._dropNode.removeClass(CSS_DROP_NODE_OVER);

				instance.fire('dragleave', event);
			},
			...
			_onDragOver: function(event) {
				...
				instance._dropNode.addClass(CSS_DROP_NODE_OVER);

				instance._dropNodeHideTask.delay(50);
			...
			_onDrop: function(event) {
				...
				instance._dropNodeHideTask.delay(0);
			...
			_renderFileSelect: function() {
				...
				if (!instance._fileSelect) {
					...
					var input = formSelect.one('input');
					var width = input.get('offsetWidth') - 30;
					var height = input.get('offsetHeight') / 2;
					...
					container.on(
						'mousemove',
						function(event) {
							xy[0] = event.pageX - width;
							xy[1] = event.pageY - height;
					...
					input.on(
						'change',
						function(event) {
							var files = event.currentTarget.attr('files');

							A.some(files.getDOM(), instance._addFile, instance);
					...
					if (instance.canSupportDD()) {
						var dropNode = instance._dropNode = A.Node.create(TPL_DROP_NODE);

						var body = A.getBody();

						dropNode.setContent(instance.get('strings.DROP_MESSAGE'));

						body.prepend(dropNode);

						body.on('dragenter', instance._onDragEnter, instance);
						body.on('dragleave', instance._onDragLeave, instance);

						instance._dropNodeHideTask = new A.DelayedTask(dropNode.hide, dropNode);

						var stopDragging = instance._stopDragging;

						body.on('dragover', stopDragging, instance);
						body.on('drop', stopDragging, instance);

						dropNode.on('dragover', instance._onDragOver, instance);
						dropNode.on('drop', instance._onDrop, instance);
					}
				}
			},
...
}, '@VERSION@' ,{requires:['aui-base','aui-task-manager','stylesheet','uploader'], skinnable:true});
